 ...
 * Modified Version from https://rishikeshdarandale.github.io/aws-http/assets/javascripts/vendor/bootstrap-toc.min.js
 */
(function($){'use strict';window.Toc={helpers:{findOrFilter:function($el,selector){var $descendants=$el.find(selector);return $el.filter(selector).add($descendants).filter(':not([data-toc-skip])')},generateUniqueIdBase:function(el){var text=$(el).text();var anchor=text.trim().toLowerCase().replace(/[^A-Za-z0-9]+/g,'-');return anchor||el.tagName.toLowerCase()},generateUniqueId:function(el){var anchorBase=this.generateUniqueIdBase(el);for(var i=0;;i++){var anchor=anchorBase;if(i>0){anchor+='-'+i}
